...
const analyzeBundle = () => {
  const buildOutput = execSync('cd apps/dashboard-web && ANALYZE=true next build', { encoding: 'utf8' });
...
const analyzeCoupling = async () => {
  ...
  const dependencies = JSON.parse(
    execSync('npx madge --json apps/', { encoding: 'utf8' })
  );
  
  // 2. Calcular métricas
  const metrics: CouplingMetrics[] = Object.entries(dependencies).map(([module, deps]) => {
    const fanOut = (deps as string[]).length;
    const fanIn = Object.values(dependencies)
      .filter(d => (d as string[]).includes(module)).length;
  ...
  const problems = metrics.filter(m => 
