...
import { Flag, MessageSquare, ImageIcon, Video, CheckCircle, XCircle, AlertTriangle } from "lucide-react"
...
export function ContentModeration() {
  ...
  const moderateContent = (id: string, action: "approve" | "reject") => {
    setReports((prev) =>
      prev.map((report) =>
        report.id === id ? { ...report, status: action === "approve" ? "approved" : "rejected" } : report,
      ),
    )
  }
              ...
              <Alert key={report.id} className="p-3">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-muted-foreground">{report.timestamp.toLocaleString("es-ES")}</span>
                  </div>
