...
export function Header({ activeSection, setActiveSection, metrics }: HeaderProps) {
  ...
  return (
    <motion.header
      ...
      className="relative sticky top-0 z-50 overflow-hidden border-b border-slate-200/50 bg-white/70 backdrop-blur-md"
    >
      {/* Texturas decorativas en el header que responden a métricas */}
      <div className="pointer-events-none absolute inset-0 opacity-5">
        <motion.div
          className="absolute right-0 top-0 h-32 w-32 bg-gradient-to-bl from-yellow-600/40 to-transparent"
          animate={{
            opacity: [0.3, 0.6, 0.3],
        ...
        />
        <motion.div
          className="absolute bottom-0 left-0 h-24 w-24 bg-gradient-to-tr from-fuchsia-500/30 to-transparent"
          animate={{
            opacity: [0.2, 0.5, 0.2],
            rotate: [0, 360],
          }}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <motion.div
              ...
              className="relative rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 px-4 py-2 font-bold text-white"
            >
              A4CO Dashboard
              ...
              return (
                <motion.div key={item.id} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  ...
                  >
                    ...
                    <motion.div
                      className={`h-2 w-2 rounded-full ${getIndicatorColor(item.indicator)}`}
                      animate={{
          ...
          <div className="flex items-center space-x-4">
            ...
            {metrics.recommendations.campaignActivity === 'peak' && (
              <motion.div
                className="flex items-center space-x-1 text-sm text-orange-600"
