    ...
    (0, react_1.useEffect)(() => {
        const interval = setInterval(() => {
            setMetrics(prev => {
                ...
                if (newActiveUsers > 1500)
                    activityLevel = 'critical';
                else if (newActiveUsers > 1200)
                    activityLevel = 'high';
                else if (newActiveUsers > 900)
                    activityLevel = 'medium';
                else
                    activityLevel = 'low';
                // Determinar actividad de campaña basada en ofertas enviadas
                let campaignActivity = 'active';
                if (prev.recommendations.sentOffers > 15)
                    campaignActivity = 'peak';
                else if (prev.recommendations.sentOffers > 8)
                    campaignActivity = 'high';
                else if (prev.recommendations.sentOffers > 3)
                    campaignActivity = 'active';
                else
                    campaignActivity = 'inactive';
                // Determinar sentimiento basado en rating promedio
                let sentiment = 'neutral';
                if (prev.comments.averageRating >= 4.5)
                    sentiment = 'excellent';
                else if (prev.comments.averageRating >= 3.5)
                    sentiment = 'positive';
                else if (prev.comments.averageRating >= 2.5)
                    sentiment = 'neutral';
                else
                    sentiment = 'negative';
                return {
                    ...
                    comments: {
                        ...
                        sentiment,
                        engagementLevel: Math.max(0, Math.min(100, prev.comments.engagementLevel + Math.floor(Math.random() * 6 - 3))),
                    },
                    settings: {
                        ...prev.settings,
                        systemHealth: Math.max(70, Math.min(100, prev.settings.systemHealth + Math.floor(Math.random() * 4 - 2))),
                        userSatisfaction: Math.max(60, Math.min(100, prev.settings.userSatisfaction + Math.floor(Math.random() * 4 - 2))),
                        stabilityIndex: prev.settings.systemHealth > 90
                            ? 'optimal'
                            : prev.settings.systemHealth > 80
                                ? 'stable'
                                : 'unstable',
                    },
                };
...
//# sourceMappingURL=use-dashboard-metrics.js.map