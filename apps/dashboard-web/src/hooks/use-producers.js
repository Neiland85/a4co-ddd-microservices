...
exports.useProducers = useProducers;
        ...
        const fetchProducers = async () => {
            ...
            try {
                ...
                if (filters.categories.length > 0) {
                    filteredProducers = filteredProducers.filter(p => filters.categories.includes(p.category));
                }
                if (filters.maxDistance > 0) {
                    filteredProducers = filteredProducers.filter(p => (p.distance || 0) <= filters.maxDistance);
                }
                if (filters.searchQuery) {
                    filteredProducers = filteredProducers.filter(p => p.name.toLowerCase().includes(filters.searchQuery.toLowerCase()) ||
...
//# sourceMappingURL=use-producers.js.map