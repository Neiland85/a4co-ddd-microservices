...
async function GET(request) {
    ...
    try {
        ...
        if (location) {
            filteredOpportunities = filteredOpportunities.filter(opp => opp.location.toLowerCase().includes(location.toLowerCase()));
        }
        // Filtrar por categoría de producto si se especifica
        if (category) {
            filteredOpportunities = filteredOpportunities.filter(opp => opp.products.some(product => product.category === category));
        }
