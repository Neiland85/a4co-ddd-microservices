...
describe('Money', () => {
    it('should create a valid Money object', () => {
        const money = new product_entity_1.Money(100, 'USD');
        expect(money.amount).toBe(100);
        expect(money.currency).toBe('USD');
    });
    it('should throw an error for a negative amount', () => {
        expect(() => new product_entity_1.Money(-10, 'USD')).toThrow('Money amount cannot be negative');
    });
    it('should throw an error for an invalid currency', () => {
        expect(() => new product_entity_1.Money(100, 'US')).toThrow('Currency must be a valid 3-letter code');
    });
    it('should correctly identify equal Money objects', () => {
        const moneyA = new product_entity_1.Money(100, 'USD');
        const moneyB = new product_entity_1.Money(100, 'USD');
        expect(moneyA.equals(moneyB)).toBe(true);
    });
    it('should correctly identify unequal Money objects', () => {
        const moneyA = new product_entity_1.Money(100, 'USD');
        const moneyB = new product_entity_1.Money(101, 'USD');
        const moneyC = new product_entity_1.Money(100, 'EUR');
    ...
    it('should add two Money objects of the same currency', () => {
        const moneyA = new product_entity_1.Money(100, 'USD');
        const moneyB = new product_entity_1.Money(50, 'USD');
    ...
    it('should throw an error when adding different currencies', () => {
        const moneyA = new product_entity_1.Money(100, 'USD');
        const moneyB = new product_entity_1.Money(50, 'EUR');
...
describe('ProductImage', () => {
    it('should create a valid ProductImage object', () => {
        const image = new product_entity_1.ProductImage('https://example.com/image.jpg', 'alt text', product_entity_1.ImageType.GALLERY, true, 1);
        expect(image.url).toBe('https://example.com/image.jpg');
        expect(image.altText).toBe('alt text');
        expect(image.type).toBe(product_entity_1.ImageType.GALLERY);
        expect(image.isPrimary).toBe(true);
        expect(image.sortOrder).toBe(1);
    });
    it('should throw an error for an invalid URL', () => {
        expect(() => new product_entity_1.ProductImage('invalid-url')).toThrow('Invalid image URL');
    });
});
describe('ProductSpecification', () => {
    it('should create a valid ProductSpecification object', () => {
        const spec = new product_entity_1.ProductSpecification('Weight', '1kg', product_entity_1.SpecificationType.TEXT, 'kg', 'General');
        expect(spec.name).toBe('Weight');
        expect(spec.value).toBe('1kg');
        expect(spec.type).toBe(product_entity_1.SpecificationType.TEXT);
        expect(spec.unit).toBe('kg');
        expect(spec.category).toBe('General');
    });
    it('should throw an error for an empty name', () => {
        expect(() => new product_entity_1.ProductSpecification(' ', 'value')).toThrow('Specification name cannot be empty');
    });
    it('should throw an error for an empty value', () => {
        expect(() => new product_entity_1.ProductSpecification('name', ' ')).toThrow('Specification value cannot be empty');
    });
});
describe('Dimensions', () => {
    it('should create a valid Dimensions object', () => {
        const dims = new product_entity_1.Dimensions(10, 20, 30, 'cm');
    ...
    it('should throw an error for non-positive dimensions', () => {
        expect(() => new product_entity_1.Dimensions(0, 20, 30)).toThrow('Dimensions must be positive numbers');
        expect(() => new product_entity_1.Dimensions(10, -5, 30)).toThrow('Dimensions must be positive numbers');
    });
    it('should calculate the correct volume', () => {
        const dims = new product_entity_1.Dimensions(2, 3, 4);
...
describe('ProductVariant', () => {
    let price;
    beforeEach(() => {
        price = new product_entity_1.Money(100, 'USD');
    });
    it('should create a valid ProductVariant object', () => {
        const variant = new product_entity_1.ProductVariant('1', 'Variant 1', 'SKU001', price, { color: 'red' }, 10, 1, new product_entity_1.Dimensions(1, 2, 3), true, true);
        ...
        expect(variant.dimensions).toBeInstanceOf(product_entity_1.Dimensions);
        expect(variant.isActive).toBe(true);
        expect(variant.isDefault).toBe(true);
    });
    it('should throw an error for an empty name', () => {
        expect(() => new product_entity_1.ProductVariant('1', ' ', 'SKU001', price, {})).toThrow('Variant name cannot be empty');
    });
    it('should throw an error for an empty SKU', () => {
        expect(() => new product_entity_1.ProductVariant('1', 'Variant 1', ' ', price, {})).toThrow('Variant SKU cannot be empty');
    });
    it('should throw an error for a negative stock quantity', () => {
        expect(() => new product_entity_1.ProductVariant('1', 'Variant 1', 'SKU001', price, {}, -1)).toThrow('Stock quantity cannot be negative');
    });
    it('should update the price', () => {
        const variant = new product_entity_1.ProductVariant('1', 'Variant 1', 'SKU001', price, {});
        const newPrice = new product_entity_1.Money(120, 'USD');
        variant.updatePrice(newPrice);
        expect(variant.price).toEqual(newPrice);
    });
    it('should update the stock quantity', () => {
        const variant = new product_entity_1.ProductVariant('1', 'Variant 1', 'SKU001', price, {}, 10);
...
describe('Product', () => {
    ...
    it('should create a valid Product object', () => {
        const product = new product_entity_1.Product('1', 'Product 1', 'Description', 'SKU001', price, 'artisan1', 'category1', 'product-1');
        expect(product.id).toBe('1');
        expect(product.name).toBe('Product 1');
        expect(product.status).toBe(product_entity_1.ProductStatus.DRAFT);
        expect(product.availability).toBe(product_entity_1.ProductAvailability.AVAILABLE);
        expect(product.domainEvents.length).toBe(1);
    });
    it('should throw an error for an empty name', () => {
        expect(() => new product_entity_1.Product('1', ' ', 'Description', 'SKU001', price, 'artisan1', 'category1', 'product-1')).toThrow('Product name cannot be empty');
    });
    it('should publish a product', () => {
        const product = new product_entity_1.Product('1', 'Product 1', 'Description', 'SKU001', price, 'artisan1', 'category1', 'product-1');
        product.addImage(new product_entity_1.ProductImage('https://example.com/image.jpg'));
        product.publish();
        expect(product.status).toBe(product_entity_1.ProductStatus.PUBLISHED);
    ...
    it('should archive a product', () => {
        const product = new product_entity_1.Product('1', 'Product 1', 'Description', 'SKU001', price, 'artisan1', 'category1', 'product-1');
        product.archive();
        expect(product.status).toBe(product_entity_1.ProductStatus.ARCHIVED);
        expect(product.availability).toBe(product_entity_1.ProductAvailability.DISCONTINUED);
    });
    it('should update the price', () => {
        const product = new product_entity_1.Product('1', 'Product 1', 'Description', 'SKU001', price, 'artisan1', 'category1', 'product-1');
        const newPrice = new product_entity_1.Money(120, 'USD');
    ...
    it('should add and remove a variant', () => {
        const product = new product_entity_1.Product('1', 'Product 1', 'Description', 'SKU001', price, 'artisan1', 'category1', 'product-1');
        const variant = new product_entity_1.ProductVariant('v1', 'Variant', 'SKU002', price, {});
