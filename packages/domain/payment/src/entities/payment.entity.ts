...
export class Payment extends AggregateRoot {
    ...
    const payment = new Payment(
      {
        ...
        stripePaymentIntentId: Payment.normalizeStripePaymentIntent(
          primitives.stripePaymentIntentId,
        ),
      },
      {
  ...
  public process(): void {
    ...
    if (!this._status.canTransitionTo(PaymentStatusValue.PROCESSING)) {
      throw new Error(
        `Payment ${this._paymentId.value} cannot transition to PROCESSING from ${this._status.value}`,
      );
    }
  ...
  public markAsSucceeded(stripePaymentIntentId: string): void {
    ...
    if (!this._status.canTransitionTo(PaymentStatusValue.SUCCEEDED)) {
      throw new Error(
        `Payment ${this._paymentId.value} cannot transition to SUCCEEDED from ${this._status.value}`,
      );
    }
  ...
  public refund(refundAmount?: Money, reason?: string): void {
    ...
    if (!this._status.canTransitionTo(PaymentStatusValue.REFUNDED)) {
      throw new Error(
        `Payment ${this._paymentId.value} cannot transition to REFUNDED from ${this._status.value}`,
      );
    }
    ...
    this.recordDomainEvent(
      new PaymentRefundedEvent({
        ...
        metadata: { ...this._metadata, refundReason: reason },
        stripePaymentIntentId: this.stripePaymentIntentId,
      }),
    );
  }
