...
const observability = (0, observability_1.initializeObservability)({
    ...
    tracing: {
        enabled: true,
        exporterType: process.env.TRACE_EXPORTER || 'console',
...
app.use(express_1.default.json()); // 4. Body parser
...
class UserService {
    ...
    async createUser(userData) {
        ...
        observability_1.TracingUtils.addEvent('user.validation.start');
        // Validación
    ...
    async notifyUserCreated(user) {
        ...
        try {
            ...
            const response = await httpClient.post('http://notification-service:3000/notifications', {
                type: 'USER_WELCOME',
                userId: user.id,
                email: user.email,
                data: {
