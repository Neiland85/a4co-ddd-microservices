...
export function AnimatedCircles({ activeSection, metrics }: AnimatedCirclesProps) {
  ...
  const getCircleAnimation = useCallback((circle: typeof circles[0]) => ({
    x: [
      `${circle.initialX}vw`,
  ...
  const getCircleTransition = useCallback((circle: typeof circles[0]) => ({
  ...
  const renderCircle = useCallback((circle: typeof circles[0]) => (
    <motion.div
    ...
    />
  ), [activeSection, animationParams.intensity, animationParams.opacity, getCircleAnimation, getCircleTransition])
