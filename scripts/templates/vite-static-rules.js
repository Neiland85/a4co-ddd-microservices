  ...
  rules: {
    'no-insecure-vite-static-config': {
      meta: {
        ...
        docs: {
          description:
            'Detects insecure Vite static file serving configurations that could lead to information disclosure',
          category: 'Security',
          recommended: true,
        },
        schema: [
        ...
        messages: {
          missingFsStrict:
            'Vite server.fs.strict should be true to prevent directory traversal attacks',
          missingFsAllow:
            'Vite server.fs.allow should be configured to restrict file system access',
          ...
          insecureFsConfig:
            'Vite server.fs configuration is insecure - missing strict mode and allow restrictions',
      ...
      create(context) {
        ...
        return {
          ...
          VariableDeclarator(node) {
            if (node.id.type === 'Identifier' && node.id.name.toLowerCase().includes('vite')) {
              if (node.init) {
          ...
          Property(node) {
            if (
              (node.key.name === 'vite' || node.key.value === 'vite') &&
    ...
    'no-html-static-serving': {
      meta: {
        ...
        docs: {
          description:
            'Warns about serving HTML files statically which could lead to information disclosure',
          category: 'Security',
        },
        schema: [],
        messages: {
          htmlStaticServing:
            'Serving HTML files statically may expose sensitive information. Consider using proper routing instead.',
        },
      },
      ...
      create() {
        return {
          Literal(node) {
            if (typeof node.value === 'string' && node.value.match(/\.(html?|ejs|pug|jade)$/i)) {
