  ...
  rules: {
    'no-dangerous-micromatch-patterns': {
      meta: {
        ...
        docs: {
          description:
            'Detects potentially dangerous micromatch patterns that could lead to ReDoS attacks',
          category: 'Security',
          recommended: true,
        },
        schema: [
        ...
        messages: {
          dangerousPattern:
            'Micromatch pattern "{{pattern}}" has {{riskLevel}} risk level (complexity: {{complexity}}). {{recommendations}}',
          excessiveWildcards:
            'Pattern contains excessive wildcards ({{count}} stars) which may cause performance issues',
          nestedGroups: 'Pattern contains nested groups which significantly increase ReDoS risk',
          multipleAlternations:
            'Pattern contains multiple alternations ({{count}}) that can lead to exponential backtracking',
        },
      },
        ...
        return {
          ...
          CallExpression(node) {
            if (
              ...
              ['match', 'isMatch', 'makeRe', 'some', 'every', 'all', 'not'].includes(
                node.callee.name
              )
            ) {
