...
import { SafeMicromatch, safeIsMatch, safeMatch } from '../utils/safe-micromatch';
  ...
  describe('MicromatchReDoSProtector', () => {
    ...
    describe('safeMatch', () => {
      it('should execute safe operations successfully', async () => {
        const result = await protector.safeMatch(() => ['file.js', 'file.ts'], ['*.js'], {
          context: 'test',
        });
