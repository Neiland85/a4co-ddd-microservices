...
class CriticalFileAnalyzer {
  constructor() {
    ...
    this.excludePaths = ['node_modules', '.turbo', 'dist', '.next'];
  }
  ...
  async analyzeFiles(files) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        console.warn(`⚠️  Error analizando ${files[i]}: ${error.message}`);
      }
    }
  ...
  calculatePriority(lines, complexity, issueCount) {
  ...
  async analyzeTechDebt() {
    ...
    try {
      const todoCount = parseInt(execSync('grep -r "TODO\\|FIXME\\|HACK" apps/ --include="*.ts" --include="*.tsx" 2>/dev/null | wc -l', { encoding: 'utf8' }).trim()) || 0;
      const anyTypeCount = parseInt(execSync('grep -r "\\bany\\b" apps/ --include="*.ts" --include="*.tsx" 2>/dev/null | wc -l', { encoding: 'utf8' }).trim()) || 0;
      const consoleLogCount = parseInt(execSync('grep -r "console\\." apps/ --include="*.ts" --include="*.tsx" 2>/dev/null | wc -l', { encoding: 'utf8' }).trim()) || 0;
      const tsIgnoreCount = parseInt(execSync('grep -r "@ts-ignore" apps/ --include="*.ts" --include="*.tsx" 2>/dev/null | wc -l', { encoding: 'utf8' }).trim()) || 0;
      ...
      console.warn('⚠️ Error calculando deuda técnica:', error.message);
      return {
        todoCount: 0,
        anyTypeCount: 0,
        consoleLogCount: 0,
  ...
  generateRecommendations(criticalFiles, techDebt) {
...
module.exports = { CriticalFileAnalyzer };